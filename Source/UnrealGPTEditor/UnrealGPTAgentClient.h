...
private:
	...
	/** Apply authentication headers from API key or Codex auth settings */
	bool ApplyAuthHeaders(TSharedRef<IHttpRequest> Request, FString& OutError) const;

	/** Refresh Codex ChatGPT auth and retry the cached request once. */
	bool RefreshCodexAuthAndRetry(const FString& RequestBody);

	void OnCodexAuthRefreshResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful);
	...
	/** Determine reasoning effort based on message complexity (returns "low", "medium", or "high") */
	FString DetermineReasoningEffort(const FString& UserMessage, const TArray<FString>& ImageBase64) const;
